 ...
 * - F-002-RQ-003: Registers exactly one application handler, for the GET method
 ...
 * 
 * Integration Pattern:
 * - This router is never mounted by app.js directly; the path is composed over two hops
 * - Hop 1 - routes/index.js imports this router: const helloRouter = require('./hello.js')
 * - Hop 1 - routes/index.js mounts router: router.use('/hello', helloRouter)
 * - Hop 2 - app.js imports the aggregator: const routes = require('./routes')
 * - Hop 2 - app.js mounts router: app.use('/', routes)
 * - Path composition: '/' (app.js) + '/hello' (routes/index.js) + '/' (this router) = /hello
 * - Final endpoint accessible at: GET /hello
 * 
 * Method Behaviour of the Single Registered Route:
 * - GET /hello: 200 with the body 'Hello world' - the one handler registered in this file
 * - HEAD /hello: 200 with headers only - generated by Express for the registered GET route
 * - OPTIONS /hello: 200 with 'Allow: GET, HEAD' - also generated by Express
 * - POST, PUT, PATCH, DELETE and TRACE on /hello match no handler and fall through to
 *   Express's own default 404 response; this file installs no method gate of its own
 * 
 * HEAD and OPTIONS are framework-generated responses for that single GET registration rather
 * than additional routes, so the application still publishes exactly one endpoint.
 * 
